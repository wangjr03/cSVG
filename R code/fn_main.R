#main code : spp=TRUE: spatial+ step applied. control=TRUE: covariates are added.
#spp=TRUE,control=TRUE : DA-SVG final
#spp=FALSE, control=TRUE: DEC-SVG
#spp=FALSE,control=FALSE: SKAT-SVG.

#update: changed prob threshold to 0.01 in marginal correlation
#update: added extra restriction in margcorr

#thres_step1 options: any no or "standard".
#method_step1 options: "SIS" or "rrcs" or "MargcorTest" OR "Enet" OR"SIS+Enet"
fn_cSVG<-function(data_mat,loc_mat,method_step1="MargcorTest",thres_step1=10,spp=TRUE,control=TRUE){
...
if (!require(SKAT)) {
  ...
  #}else if(!require(devtools)){
    ...
    #stop("devtools package not installed")
  #}else if(!require(screening)){
    #print("installing notoncran because you don't have it...")
    #devtools::install_github('wwrechard/screening')
    #require(screening)
    #stop("screening package not installed")
  }else {
  }
...
##calculating kernel parameters(code from SPARK:https://github.com/xzhoulab/SPARK/blob/master/R/utilties.R)
# and from https://github.com/xzhoulab/SPARK/blob/master/R/vctesting.R
...
#Kernel to be considered #copied from spark https://github.com/xzhoulab/SPARK/blob/master/R/vctesting.R
...
exp_f=data_mat
coord_df=loc_mat
m=nrow(exp_f) #no of genes
N=nrow(coord_df) #no of spots
...
#if(dim(exp_f)[2]!=N){
#  stop("Make sure the input matrices are correct.")
#}

fn_res<-function(X){
  require(mgcv)
  k_sp<-300
  f_X_hat<-gam(X~s(coord_df[,1],coord_df[,2],k=k_sp,fx=TRUE),method="GCV.Cp")$fitted.values #,method="REML"
  r_X<-X-f_X_hat
  return(r_X)
}

#if(spp==TRUE){
#exp_spp=apply(exp_f,1,fn_res)
#exp_spp=t(exp_spp)
#}
################

p_val_mat=matrix(0,nrow=nrow(exp_f),ncol=10)
...
for(g in genes){
    ...
    data_mat1= dep[out$screen,]
      ...
      res=apply(dep,1,fn_corTest)
      res=p.adjust(res, method = "BY")
      ind1=which(res<0.01) #no need to adjust. look at next line
  ...
  for(ikernel in c(1:5) ){
      ...
      p_val_mat[ind,]=p_val_all
      #print(p_val_mat)
}

rownames(p_val_mat) <- rownames(exp_f)
