...
fn_cSVG_par<-function(data_mat,loc_mat,method_step1="MargcorTest",thres_step1="standard",control=FALSE,ncores=7){
  ...
  print("dim of data matrix is:")
  print(dim(exp_f))
  print("dim of coordinate matrix is:")
  print(dim(coord_df))
  ...
  #g=genes[gene_index]
    ...
    library(parallel)
    new=parallel::mclapply(c(1:5),function(ikernel){cat(paste0("## testing Gaussian kernel: ",ikernel,"...\n"))
      ...
      SKAT2<-SKAT.linear.Other(obj1$res,Z,obj1$X1,kernel = kernel_mat,weights = NULL,obj1$s2,method = 'davies',obj1$res.out,obj1$n.Resampling)
      
      return(c(SKAT1$p.value,SKAT1$p.value)) 
      },mc.cores=ncores)
    
    p_val_mat[ind,]=unlist(new)
